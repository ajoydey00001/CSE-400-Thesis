...
if __name__ == "__main__":
	sys.stdout = open('/home/uiu/nlp/GEC/BanglaGEC/outputs/pipelineBanglaT5.txt', 'w')
	...
	for (input_ids, attention_mask, target_ids) in tqdm(test_loader):
		...
		# print(f"prd_text: {prd_text}")
		# print(f"trg_text: {trg_text}")
		prd_text = punctuations_preprocessing(prd_text)
		trg_text = punctuations_preprocessing(trg_text)
	...
	# for idx in tqdm(range(1000)):
		...
		# ----------------------------
		# Preprocess pred_text: 
		# i. remove whitespace between word and punctuation
		# TODO
		src_text = punctuations_preprocessing(src_text)
		prd_text = punctuations_preprocessing(prd_text)
		trg_text = punctuations_preprocessing(trg_text)
		# ----------------------------
	...
	acc = accuracy_score(y_true=refs4BERTscore, y_pred=preds)
	pr = precision_score(y_true=refs4BERTscore, y_pred=preds, average='micro')
	re = recall_score(y_true=refs4BERTscore, y_pred=preds, average='micro')
	f1 = fbeta_score(y_true=refs4BERTscore, y_pred=preds, average='micro', beta=1)
	f05 = fbeta_score(y_true=refs4BERTscore, y_pred=preds, average='micro', beta=0.5)
